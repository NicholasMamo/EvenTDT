...
"""
# FILENAME = "BRABEL.json"
# FILENAME = "BELENG.json"
# FILENAME = "FRACRO.json"
FILENAME = "CommunityShield.json"
IDF = "idf.json"
...
async def create_stream():
	...
	with open(os.path.join("/mnt/data/twitter", FILENAME)) as f:
		listener = SimulatedFileListener(queue, f, speed=SPEED, max_time=EVENT_DURATION/SPEED, skip_time=SKIP_TIME) # skip the first hour
...
async def consume_queue(consumer):
	...
	topics = await consumer.run(max_time=EVENT_DURATION, max_inactivity=60) # with the ELD consumer
...
idf.close()
consumer = SimulatedELDConsumer(queue, time_window=TIME_WINDOW, filter_words=FILTER_WORDS, idf=idf_dict)
