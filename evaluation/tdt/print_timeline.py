...
if len(sys.argv) >= 2:
	...
	with open(os.path.join(OUT_DIR, "temp.txt"), "w") as o:
		...
		with open("/mnt/evaluation/tdt/timeline/%s_timeline.json" % ('_'.join(NAME)), "r") as f:
			print("/mnt/evaluation/tdt/timeline/%s_timeline.json" % ('_'.join(NAME)))
			for line in f:
				data = json.loads(line)
				summary = Summary.from_array(data)
				o.write("%s: %s" % (datetime.fromtimestamp(summary.created_at()).strftime('%Y-%m-%d %H:%M:%S'),
					summary.generate_summary(cleaner=tweet_cleaner.TweetCleaner)))
				o.write("\n")
