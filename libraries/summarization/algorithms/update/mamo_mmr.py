"""
Implementation of Mamo et al.'s time-aware MMR (unpublished).
...
class FragmentedMMR(FragmentedUpdateSummarization):
	"""
	...
	In this way, the MMR's query is made up of these recent terms.
	The query terms are weighted according to their relevance and the popularity of the cluster.
	...
	def __init__(self, time_window=120, scorer=scorer.Scorer):
		"""
	...
	def add_cluster(self, cluster, breaking_terms, timestamp, wait_period=90, summarization_min_score=0, *args, **kwargs):
		...
		if timestamp - self._summary.last_updated() > self._time_window:
			"""
