...
class TestLinguisticExtractor(unittest.TestCase):
    ...
    def test_remove_parentheses_original(self):
        ...
        self.assertEqual("Pope Francis (Latin: Franciscus; Italian: Francesco; Spanish: Francisco; born Jorge Mario Bergoglio,[b] 17 December 1936) is the head of the Catholic Church and sovereign of the Vatican City State since 2013.", sentence)
        self.assertEqual({ 'is': { 'head' }, 'is_of': { 'catholic church', 'sovereign', 'vatican city state' }, 'is_since':{ '2013' } },
    ...
    def test_remove_parentheses_none(self):
        ...
        self.assertEqual({ 'is': { 'continent' }, 'recognised_as': { 'part' }, 'recognised_of': { 'eurasia' }, 'located_in': { 'northern hemisphere', 'eastern hemisphere' } },
                         LinguisticExtractor(head_only=False).extract(sentence, remove_parentheses=True).attributes)
        self.assertEqual({ 'is': { 'continent' }, 'recognised_as': { 'part' }, 'recognised_of': { 'eurasia' }, 'located_in': { 'northern hemisphere', 'eastern hemisphere' } },
                         LinguisticExtractor(head_only=True).extract(sentence, remove_parentheses=True).attributes)
    ...
    def test_remove_parentheses_simple(self):
        ...
        self.assertEqual({ 'is': { 'french professional footballer' }, 'plays_as': { 'forward' }, 'plays_for': { 'paris saint-germain', 'france national team' } },
                         LinguisticExtractor(head_only=False).extract(sentence, remove_parentheses=True).attributes)
        self.assertEqual({ 'is': { 'footballer' }, 'plays_as': { 'forward' }, 'plays_for': { 'paris saint-germain', 'team' } },
                         LinguisticExtractor(head_only=True).extract(sentence, remove_parentheses=True).attributes)
    ...
    def test_remove_parentheses_multiple(self):
        ...
        self.assertEqual({ 'is': { 'capital', 'most populous city' }, 'is_of': { 'ukraine' } },
                         LinguisticExtractor(head_only=False).extract(sentence, remove_parentheses=True).attributes)
        self.assertEqual({ 'is': { 'capital', 'city' }, 'is_of': { 'ukraine' } },
                         LinguisticExtractor(head_only=True).extract(sentence, remove_parentheses=True).attributes)
    ...
    def test_remove_parentheses_keep(self):
        ...
        profile = extractor.extract(sentence, remove_parentheses=True)
        self.assertEqual({ 'is': { 'second child', 'only daughter' }, 'is_of': { 'queen elizabeth ii', 'prince philip', 'duke', 'edinburgh' } },
                         LinguisticExtractor(head_only=False).extract(sentence, remove_parentheses=True).attributes)
        self.assertEqual({ 'is': { 'child', 'daughter' }, 'is_of': { 'queen elizabeth ii', 'prince philip', 'duke', 'edinburgh' } },
                         LinguisticExtractor(head_only=True).extract(sentence, remove_parentheses=True).attributes)

        sentence = "Anne, Princess Royal KG, KT, GCVO, QSO, CD (Anne Elizabeth Alice Louise; born 15 August 1950), is the second child and only daughter of Queen Elizabeth II and Prince Philip, Duke of Edinburgh."
        self.assertEqual({ 'born': { '15 august 1950' }, 'is': { 'second child', 'only daughter' }, 'is_of': { 'queen elizabeth ii', 'prince philip', 'duke', 'edinburgh' } },
                         LinguisticExtractor(head_only=False).extract(sentence, remove_parentheses=False).attributes)
        self.assertEqual({ 'born': { '15 august 1950' }, 'is': { 'child', 'daughter' }, 'is_of': { 'queen elizabeth ii', 'prince philip', 'duke', 'edinburgh' } },
    ...
    def test_remove_parentheses_nested(self):
        ...
        self.assertEqual({ 'known_as': { 'leo messi' }, 'is': { 'argentine professional footballer' }, 'plays_as': { 'forward' }, 'plays_for': { 'paris saint-germain' }, 'captains': { 'argentina national team' } },
                         LinguisticExtractor(head_only=False).extract(sentence, remove_parentheses=True).attributes)
        self.assertEqual({ 'known_as': { 'leo messi' }, 'is': { 'footballer' }, 'plays_as': { 'forward' }, 'plays_for': { 'paris saint-germain' }, 'captains': { 'team' } },
                         LinguisticExtractor(head_only=True).extract(sentence, remove_parentheses=True).attributes)
