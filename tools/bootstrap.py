...
def load_seed(file, max_seed=None):
    """
    ...
    :param file: The path to the seed file.
    :type file: str
    ...
    if terms.isOwn(file):
        ...
        with open(file, 'r') as f:
            seeds.extend(f.readlines())
            seeds = [ word.strip() for word in seeds ]

    max_seed = max_seed or len(seeds)
    if not seeds:
        raise ValueError("The seed list cannot be empty if it is given")
    return seeds[:max_seed]

def load_candidates(file, max_candidates=None):
    """
    ...
    :param file: The path to the candidate file.
    :type file: str
