...
BASE_NAME = "brazil_election_apd.json"
SEED_SET = ["brazil election", "brasil election"]

# script output/brazil_election_apd.txt -c 'python3 /home/memonick/data/brazil_election_apd.py'
UNDERSTANDING_FILENAME = "/mnt/data/brazil/apd/u_%s" % (BASE_NAME)
FILENAME = "/mnt/data/brazil/apd/%d_%s"
...
with open(os.path.join("/home/memonick/data", IDF), "r") as idf_file:
	...
	for tweet in data:
		...
		document = Document(dimensions=tokens, attributes={ "tokens": tokens, "text": text, "tweet": tweet }, scheme=term_weighting)
		document.normalize()
