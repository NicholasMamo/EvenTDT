...
with open(FILENAME, "w") as f:
	...
	with open(os.path.join("/home/memonick/data", IDF), "r") as idf_file:
		...
		for tweet in data:
			...
			document = Document(tokens, { "tokens": tokens, "text": tweet["text"] })
		...
		"""
		participant_detector = ParticipantDetector(corpus, EntityExtractor, SumScorer, SearchResolver, LinkExtrapolator, WikipediaPostprocessor)
		resolved, unresolved, extrapolated = participant_detector.detect(threshold=0.1,
			resolver_scheme=TFIDF(general_idf), resolver_threshold=0.1,
			extrapolator_scheme=TFIDF(general_idf), extrapolator_participants=30, extrapolator_threshold=0.01,
			postprocessor_surname_only=False)
		resolved = [ keyword.lower() for keyword in resolved if keyword.lower() not in words.words() ]
		extrapolated = [ keyword.lower() for keyword in extrapolated if keyword.lower() not in words.words() ]
